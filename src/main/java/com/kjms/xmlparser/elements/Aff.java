...
import static com.kjms.xmlparser.util.Util.*;
    ...
    public Aff(Node node) {
        try {
            ...
            if(nodeList.size()>0 && label!=null)
                ...
                this.html += htmlTagBinder(Label.ELEMENT_HTML,  new StringBuffer(this.node.getTextContent()).deleteCharAt(0).toString());

            if(nodeList.size()>0 && label==null)
                this.html += htmlTagBinder(Label.ELEMENT_HTML,  new StringBuffer(this.node.getTextContent()).toString());
            if (nodeList.size() == 0){
                this.html += htmlTagBinder("div", node.getTextContent());
            }
